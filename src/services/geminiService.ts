...
5. Generate exactly 6 tailored interview questions, categorized and ordered precisely as follows.
   IMPORTANT RULE FOR QUESTIONS: Keep ALL questions short and concise — ideally one line, maximum two lines. Do NOT write long, multi-clause questions. Be direct and simple. For example, instead of "Can you explain the concept of Promises in JavaScript and how they are used to handle asynchronous operations, especially in the context of Node.js and Express.js applications?", simply ask "What are Promises in JavaScript and how do they work?".

   IMPORTANT RULE FOR ANSWERS (idealAnswer): The idealAnswer MUST be a JSON object with exactly 3 fields:
   - "coreAnswer": A single punchy sentence — the direct answer a beginner would speak in an interview. Max 1-2 sentences.
   - "keyPoints": An array of 2-3 short strings, each one a distinct key concept or fact. Keep each point under 15 words.
   - "example": A single short real-world example, analogy, or mini code snippet. Max 1-2 sentences.
   NEVER use meta-phrases like "An ideal answer would mention...". Write as if a real candidate is speaking.
   ...
   - Question 3 (Type: "technical"): A short concept question on yet another distinct skill or technology from the resume. If not enough skills exist, ask about a tool, framework, or methodology relevant to their field.
   - Question 4 (Type: "behavioral"): A brief project challenge question — ask which feature or part of a specific project was the hardest to build and why.
   - Question 5 (Type: "behavioral"): A brief project experience question — ask the candidate to share their key learnings or experience from one of their projects.
   - Question 6 (Type: "problem-solving"): A short, beginner-level problem-solving challenge RELEVANT to the candidate's career field. For developers, give a simple coding problem with a concrete input/output example (e.g., "Write a function that finds the largest number in an array. Input: [3,7,2,9], Output: 9"). For UI/UX designers, give a short design challenge. For non-tech fields, give a domain-relevant practical problem. NEVER ask a coding question to a non-programmer. NEVER repeat "filter even numbers" — always vary the problem.
  ...
  "questions": [
    {
      ...
      "type": "technical",
      "idealAnswer": {
        "coreAnswer": "The Virtual DOM is a lightweight copy of the real DOM that React uses to efficiently update only the changed parts of the page.",
        "keyPoints": [
          "In-memory representation of the real DOM",
          "Uses diffing algorithm to detect changes",
          "Only updates changed elements, not the whole page"
        ],
        "example": "If you change one item in a list of 100, React updates only that one item instead of re-rendering all 100."
      }
    }
  ]
}`,
